...
* git remote add ```<name> <branch>``` добавление удаленного репозитория в локальный репозиторий. 
...
* git push -d ```<remote> <branch>``` удаление удаленной ветки. 
* git remote show ```<remote>``` просмотр подробной информации об указанном удаленном репозитории. 